...
class Account2166Controller extends Controller
 { 
    
    public function account_pkti2166_dash_old(Request $request)
    {
        $datenow = date('Y-m-d');
    ...
    public function account_pkti2166_pull(Request $request)
    {
        ...
        return view('account_2166.account_pkti2166_pull',[
            'startdate'     =>     $startdate,
            'enddate'       =>     $enddate,
    ...
    public function account_pkti2166_stam(Request $request)
    {
        ...
        foreach ($data as $key => $value) {
                ...
                $check = Acc_1102050101_2166::where('vn', $value->vn)->count();
                if ($check > 0) {
                # code...
                } else {
    ...
    public function account_pkti2166_detail(Request $request,$months,$year)
    {
        $datenow = date('Y-m-d');  
        $data['users'] = User::get();  
        $data = DB::select('
        ...
        return view('account_2166.account_pkti2166_detail', $data, [ 
            'data'       =>     $data,
            'months'     =>     $months,
            'year'       =>     $year
        ]);
    ...
    public function account_pkti2166_stm(Request $request,$months,$year)
    {
        ...
        $data = DB::select('
            SELECT U1.vn,U1.hn,U1.cid,U1.ptname,U1.vstdate,U1.pttype,U1.debit_total,am.Total_amount,am.STMdoc 
                ...
                group by U1.vn
        ');       
        return view('account_2166.account_pkti2166_stm', $data, [ 
            'data'          =>     $data,
            'months'        =>     $months,
            'year'          =>     $year
        ]);
    }
    public function account_pkti2166_stmnull(Request $request,$months,$year)
    {
        $datenow = date('Y-m-d');        
        $data['users'] = User::get();
        $data = DB::select('
        ...
        ');       
        return view('account_2166.account_pkti2166_stmnull', $data, [ 
            'data'          =>     $data,
            'months'        =>     $months,
            'year'          =>     $year
        ]);
    ...
    public function account_pkti2166_detail_date(Request $request,$startdate,$enddate)
    {
        $datenow = date('Y-m-d');  
        $data['users'] = User::get();  
        $data = DB::select('
            ...
            GROUP BY U1.vn
        ');      
        return view('account_2166.account_pkti2166_detail_date', $data, [ 
            'data'          =>  $data,
            'startdate'     =>  $startdate,
            'enddate'       =>  $enddate
        ]);
    }
    public function account_pkti2166_stm_date(Request $request,$startdate,$enddate)
    {
        $datenow = date('Y-m-d'); 
        $data['users'] = User::get();
        $data = DB::select('
            SELECT U1.vn,U1.hn,U1.cid,U1.ptname,U1.vstdate,U1.pttype,U1.debit_total,am.Total_amount ,am.STMdoc 
                ...
                AND am.Total_amount is not null 
        ');       
        return view('account_2166.account_pkti2166_stm_date', $data, [ 
            'data'          =>  $data,
            'startdate'     =>  $startdate,
            'enddate'       =>  $enddate
        ]);
    }
    public function account_pkti2166_stmnull_date(Request $request,$startdate,$enddate)
    {
        $datenow = date('Y-m-d'); 
        $data['users'] = User::get();
        ...
        return view('account_2166.account_pkti2166_stmnull_date', $data, [ 
            'data'          =>  $data,
            'startdate'     =>  $startdate,
            'enddate'       =>  $enddate
        ]);
